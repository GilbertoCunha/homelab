...
| `503` | The route matched and the backend is not answering. Usually the workload; also what a blocked Gateway looks like, since a project namespace denies ingress by default. The chart's `allow-gateways` policy admits only the Gateways of the namespace's environment on **port 15008**, so a route whose Gateway is not in `defaults.gateways` for that environment gets exactly this. ztunnel's logs say so: `maybe a NetworkPolicy is blocking HBONE port 15008`. See [Istio in ambient mode](../../concepts/istio.md) |
